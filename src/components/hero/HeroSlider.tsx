...
export function HeroSlider({ streams, autoPlayInterval = 5000 }: HeroSliderProps) {
  ...
  return (
    ...
    >
      <div className="container relative py-12 px-4 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-6 items-stretch min-h-[400px]">
          ...
          <div className="flex-1 min-w-0 relative group/carousel">
            <div ref={emblaRef} className="overflow-hidden lg:overflow-visible h-full">
              <div className="flex h-full items-center">
                {streams.map((stream, index) => {
                  ...
                  return (
                    <div
                      key={stream.id}
                      className="relative flex-[0_0_85%] lg:flex-[0_0_80%] min-w-0 px-4"
                      style={{ 
                        perspective: '1500px',
                    ...
                    >
                      <Link to={`/watch/live/${stream.id}`} className="block h-full">
                        <div 
                          className={cn(
                            "relative aspect-video rounded-3xl overflow-hidden transition-all duration-500 ring-1 ring-white/5",
                        ...
                        >
                          ...
                          <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-300">
                             <div className="h-12 w-12 rounded-full bg-primary flex items-center justify-center scale-90 group-hover/carousel:scale-100 transition-transform duration-300">
            ...
            <Button
              ...
              size="icon"
              onClick={scrollPrev}
              className="absolute left-2 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-black/60 backdrop-blur-md border border-white/10 opacity-0 group-hover/carousel:opacity-100 transition-all z-20"
            >
            ...
            <Button
              ...
              size="icon"
              onClick={scrollNext}
              className="absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-black/60 backdrop-blur-md border border-white/10 opacity-0 group-hover/carousel:opacity-100 transition-all z-20"
            >
          ...
          {/* Compact Info Panel */}
          <div className="w-full lg:w-[200px] h-[400px] flex">
             <div className="glass-card w-full p-5 rounded-3xl border-white/10 flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="pt-4 space-y-4">
                   ...
                   <Button asChild className="w-full h-10 rounded-xl bg-primary text-black font-bold text-xs hover:shadow-glow-primary transition-all">
