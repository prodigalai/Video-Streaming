...
const REELS_DATA = [
  {
    ...
    description: "Just another day in paradise! 🌴 #travel #vibes",
    likes: "12.5K",
    comments: "342"
  },
  {
    ...
    description: "Exclusive beat drop preview. Only for the real ones 🎵",
    likes: "8.2K",
    comments: "156"
  },
  {
    ...
    description: "That headshot was insane! 🎯 #gaming #highlights",
    likes: "25K",
    comments: "1.2K"
  },
  {
    ...
    description: "Secret pasta sauce recipe reveal 🍝",
    likes: "15K",
    comments: "890"
  },
  {
    ...
    price: 100,
    creator: { name: "Luna", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=luna" },
    description: "Backstage access at the summer festival 🎫",
    likes: "9.1K",
    comments: "230"
  }
];
             ...
             {REELS_DATA.map((reel) => {
               ...
               const needsUnlock = reel.isLocked && !isReelUnlocked;
                ...
                return (
                  <div 
                    key={reel.id} 
                    className="relative flex-shrink-0 w-44 md:w-52 aspect-[9/16] rounded-2xl overflow-hidden group snap-start border border-white/5 cursor-pointer transition-transform duration-300 hover:scale-[1.02]"
                     ...
                     {needsUnlock ? (
                       <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center space-y-3">
                          ...
                          <Button 
                            onClick={(e) => {
                              e.stopPropagation();
                              handleUnlock(reel.id, reel.price!);
                            }}
                       ...
                       <>
                         <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="h-12 w-12 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/20 scale-0 group-hover:scale-100 transition-transform duration-300">
                               <ExternalLink className="h-5 w-5 text-white ml-0.5" />
                            </div>
                         </div>
                     ...
                     {!needsUnlock && (
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                           <button className="flex flex-col items-center gap-1 group/act">
                              ...
                              <span className="text-[8px] font-bold text-white">{reel.likes || "4.2K"}</span>
                           </button>
                           <button className="flex flex-col items-center gap-1 group/act">
                              ...
                              <span className="text-[8px] font-bold text-white">{reel.comments || "128"}</span>
                           </button>
                        </div>
                     )}
                  </div>
